    ...
    static void calcFilter(double Fs, double Fa, double Fb, int M, double Att, double* dest)
    {
        ...
        for (int j = 0; j <= Np; j++)
        {
            dest[Np + j] =
                A[j] *
                Ino(Alpha *
                    std::sqrt(1.0 - (static_cast<double>(j * j) / static_cast<double>(Np * Np)))) /
                Inoalpha;
        }
        dest[Np + Np] = A[Np] * Ino(0.0) / Inoalpha; // ARM with optimizer level O3 returns NaN ==
                                                     // sqrt(1.0 - n/n), while x64 does not...
        for (int j = 0; j < Np; j++)
        {
            dest[j] = dest[M - 1 - j];
        }
...
typedef struct _Flt
{
    double coef alignas(16)[maxTap] = {
        0,
    };
    double buff alignas(16)[maxTap] = {
        0,
    };
    int TAP_SIZE = 0;
    int TAP_HALF = 0;
...
public:
    ...
    void update(SampleType** channelData, int numChannels, int numSamples)
    {
        ...
        for (int ch = 0; ch < numChannels; ch++)
        {
            for (int i = 0; i < numSamples; i++)
            {
                ...
                {
                    double pwr = Decibels::gainToDecibels(std::abs(channelData[ch][i]) *
                                                          std::abs(channelData[ch][i]));
    ...
    double getEnv(int channel)
    {
        if (channel < 0)
            return 0.0;
        if (channel >= state.size())
            return 0.0;

        if (type == Peak)
            return Decibels::decibelsToGain(state[channel]);
        else
            return std::sqrt(Decibels::decibelsToGain(state[channel]));
    }
...
protected:
    ...
    template <typename SampleType>
    void processAudio(SampleType** inputs, SampleType** outputs, int32 numChannels,
                      SampleRate getSampleRate, int32 sampleFrames);
    template <typename SampleType>
    void latencyBypass(SampleType** inputs, SampleType** outputs, int32 numChannels,
                       SampleRate getSampleRate, int32 sampleFrames);
