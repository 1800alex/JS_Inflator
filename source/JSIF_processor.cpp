...
JSIF_Processor::JSIF_Processor()
    : fInput(init_Input), fOutput(init_Output), fEffect(init_Effect), fCurve(init_Curve),
      fParamZoom(init_Zoom), fParamPhase(init_Phase), bBypass(init_Bypass), bIn(init_In),
      bClip(init_Clip), bSplit(init_Split), curvepct(init_curvepct), curveA(init_curveA),
      curveB(init_curveB), curveC(init_curveC), curveD(init_curveD), fParamOS(init_OS)
{
...
#define clear_delete(vec)                                                                          \
    ...
    for (auto& loop : upSample_2x_Lin)
        delete loop;
    for (auto& loop : upSample_4x_Lin)
        delete loop;
    for (auto& loop : upSample_8x_Lin)
        delete loop;
    for (auto& loop : dnSample_2x_Lin)
        delete loop;
    for (auto& loop : dnSample_4x_Lin)
        delete loop;
    for (auto& loop : dnSample_8x_Lin)
        delete loop;
...
//------------------------------------------------------------------------
tresult PLUGIN_API JSIF_Processor::setBusArrangements(Vst::SpeakerArrangement* inputs, int32 numIns,
...
tresult PLUGIN_API JSIF_Processor::setupProcessing(Vst::ProcessSetup& newSetup)
{
    ...
    for (int channel = 0; channel < numChannels; channel++)
    {
        ...
        for (int i = 0; i < upTap_21; i++)
            upSample_21[channel].coef[i] *= 2.0;
        for (int i = 0; i < upTap_41; i++)
            upSample_41[channel].coef[i] *= 2.0;
        for (int i = 0; i < upTap_42; i++)
            upSample_42[channel].coef[i] *= 2.0;
        for (int i = 0; i < upTap_81; i++)
            upSample_81[channel].coef[i] *= 2.0;
        for (int i = 0; i < upTap_82; i++)
            upSample_82[channel].coef[i] *= 2.0;
        for (int i = 0; i < upTap_83; i++)
            upSample_83[channel].coef[i] *= 2.0;
...
uint32 PLUGIN_API JSIF_Processor::getLatencySamples()
{
    ...
    if (fParamPhase)
    {
        if (fParamOS == overSample_1x)
            ...
            return latency_r8b_x2;
        else if (fParamOS == overSample_4x)
            return latency_r8b_x4;
        else
            return latency_r8b_x8;
    }
    else
    {
        if (fParamOS == overSample_1x)
            ...
            return latency_Fir_x2;
        else if (fParamOS == overSample_4x)
            return latency_Fir_x4;
        else
            return latency_Fir_x8;
    }
}
...
tresult PLUGIN_API JSIF_Processor::process(Vst::ProcessData& data)
{
                ...
                if (paramQueue->getPoint(numPoints - 1, sampleOffset, value) == kResultTrue)
                {
                    switch (paramQueue->getParameterId())
                    {
                        ...
                        case kParamPhase:
                            ...
                            break;
                        case kParamOS:
                            fParamOS = static_cast<overSample>(
                                Steinberg::FromNormalized<ParamValue>(value, overSample_num));
                            sendTextMessage("OS");
                            break;
                    }
                }
    ...
    // init VuMeters
    for (auto& loop : fInputVu)
        loop = init_meter;
    for (auto& loop : fOutputVu)
        loop = init_meter;
    fMeterVu = init_meter;
    ...
    else
    {
        ...
        if (bBypass)
        {
            ...
            {
                latencyBypass<Vst::Sample32>((Vst::Sample32**)in, (Vst::Sample32**)out, numChannels,
                                             SampleRate, data.numSamples);
            }
            else if (data.symbolicSampleSize == Vst::kSample64)
            {
                latencyBypass<Vst::Sample64>((Vst::Sample64**)in, (Vst::Sample64**)out, numChannels,
                                             SampleRate, data.numSamples);
            }
        ...
        else
        {
            if (data.symbolicSampleSize == Vst::kSample32)
            {
                processAudio<Vst::Sample32>((Vst::Sample32**)in, (Vst::Sample32**)out, numChannels,
                                            SampleRate, data.numSamples);
            }
            else if (data.symbolicSampleSize == Vst::kSample64)
            {
                processAudio<Vst::Sample64>((Vst::Sample64**)in, (Vst::Sample64**)out, numChannels,
                                            SampleRate, data.numSamples);
            }
...
tresult PLUGIN_API JSIF_Processor::setState(IBStream* state)
{
    ...
    if (streamer.readDouble(savedZoom) == false)
        return kResultFalse;
    if (streamer.readDouble(savedLin) == false)
        return kResultFalse;
    ...
    fOutput = savedOutput;
    fParamOS =
        static_cast<overSample>(Steinberg::FromNormalized<ParamValue>(savedOS, overSample_num));
    bClip = savedClip > 0;
    bIn = savedIn > 0;
    bSplit = savedSplit > 0;
...
tresult PLUGIN_API JSIF_Processor::getState(IBStream* state)
{
    ...
    streamer.writeDouble(
        Steinberg::ToNormalized<ParamValue>(static_cast<ParamValue>(fParamOS), overSample_num));
...
Vst::Sample64 JSIF_Processor::process_inflator(Vst::Sample64 inputSample)
{
    ...
    if (s1 >= 2.0)
        ...
        inputSample = (2.0 * s1) - s2;
    else
        inputSample =
            (curveA * s1) + (curveB * s2) + (curveC * s3) - (curveD * (s2 - (2.0 * s3) + s4));
    inputSample *= sign;
...
template <typename SampleType>
void JSIF_Processor::latencyBypass(SampleType** inputs, SampleType** outputs, int32 numChannels,
                                   Vst::SampleRate SampleRate, int32 sampleFrames)
{
    int32 latency = 0;
    if (!fParamPhase)
    {
        if (fParamOS == overSample_2x)
            latency = latency_Fir_x2;
        else if (fParamOS == overSample_4x)
            latency = latency_Fir_x4;
        else if (fParamOS == overSample_8x)
            latency = latency_Fir_x8;
    }
    else
    {
        if (fParamOS == overSample_2x)
            latency = latency_r8b_x2;
        else if (fParamOS == overSample_4x)
            latency = latency_r8b_x4;
        else if (fParamOS == overSample_8x)
            latency = latency_r8b_x8;
    }
        ...
        if (latency != latency_q[channel].size())
        {
            ...
            if (diff > 0)
                for (int i = 0; i < diff; i++)
                    latency_q[channel].push_back(0.0);
            else
                for (int i = 0; i < -diff; i++)
...
template <typename SampleType>
void JSIF_Processor::processAudio(SampleType** inputs, SampleType** outputs, int32 numChannels,
                                  Vst::SampleRate SampleRate, int32 sampleFrames)
{
    ...
    for (int32 channel = 0; channel < numChannels; channel++)
    {
        ...
        while (--samples >= 0)
        {
            ...
            else
            {
                ...
                {
                    if (fParamOS == overSample_2x)
                        Fir_x2_up(&inputSample, up_x, channel);
                    else if (fParamOS == overSample_4x)
                        Fir_x4_up(&inputSample, up_x, channel);
                    else
                        Fir_x8_up(&inputSample, up_x, channel);
                }
                else
                {
            ...
            for (int k = 0; k < oversampling; k++)
            {
                ...
                if (bSplit)
                {
                    fBand_Split[channel].LP.R =
                        fBand_Split[channel].LP.I +
                        fBand_Split[channel].LP.C * (sampleOS - fBand_Split[channel].LP.I);
                    fBand_Split[channel].LP.I =
                        2 * fBand_Split[channel].LP.R - fBand_Split[channel].LP.I;

                    fBand_Split[channel].HP.R =
                        (1 - fBand_Split[channel].HP.C) * fBand_Split[channel].HP.I +
                        fBand_Split[channel].HP.C * sampleOS;
                    fBand_Split[channel].HP.I =
                        2 * fBand_Split[channel].HP.R - fBand_Split[channel].HP.I;

                    Vst::Sample64 inputSample_L = fBand_Split[channel].LP.R;
                    Vst::Sample64 inputSample_H = sampleOS - fBand_Split[channel].HP.R;
                    Vst::Sample64 inputSample_M =
                        fBand_Split[channel].HP.R - fBand_Split[channel].LP.R;

                    up_y[k] = process_inflator(inputSample_L) +
                              process_inflator(inputSample_M * fBand_Split[channel].G) *
                                  fBand_Split[channel].GR +
            ...
            else
            {
                ...
                {
                    if (fParamOS == overSample_2x)
                        Fir_x2_dn(up_y, &inputSample, channel);
                    else if (fParamOS == overSample_4x)
                        Fir_x4_dn(up_y, &inputSample, channel);
                    else
                        Fir_x8_dn(up_y, &inputSample, channel);
                }
                else
                {
                    double* dnSample_buff;
                    if (fParamOS == overSample_2x)
                        dnSample_2x_Lin[channel]->process(up_y, oversampling, dnSample_buff);
                    else if (fParamOS == overSample_4x)
                        dnSample_4x_Lin[channel]->process(up_y, oversampling, dnSample_buff);
                    else
                        dnSample_8x_Lin[channel]->process(up_y, oversampling, dnSample_buff);
...
void JSIF_Processor::HB_upsample(Flt* filter, Vst::Sample64* out)
{
    ...
    {
        acc += filter->coef[coef] *
               (filter->buff[buff] + filter->buff[filter->TAP_HALF - buff - filter->START]);
    }
