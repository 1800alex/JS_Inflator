...
public:
    ...
    IdStringPtr getViewName() const SMTG_OVERRIDE
    {
        return "My Vu Meter";
    }
    ...
    IdStringPtr getBaseViewName() const SMTG_OVERRIDE
    {
               ...
               const IUIDescription* description) const SMTG_OVERRIDE
    {
        ...
        if (attr)
            vuMeter->setStyle(*attr == UIViewCreator::strVertical ? MyVuMeter::kVertical
                                                                  : MyVuMeter::kHorizontal);

        CColor color;
        if (UIViewCreator::stringToColor(attributes.getAttributeValue(kAttrVuOnColor), color,
                                         description))
            vuMeter->setVuOnColor(color);
        if (UIViewCreator::stringToColor(attributes.getAttributeValue(kAttrVuOffColor), color,
                                         description))
                               ...
                               ConstStringPtrList& values) const SMTG_OVERRIDE
    {
        if (attributeName == UIViewCreator::kAttrOrientation)
        {
            return UIViewCreator::getStandardAttributeListValues(UIViewCreator::kAttrOrientation,
                                                                 values);
        }
        return false;
    }
...
template <> void JSIF_Controller::UIVuMeterController::updateVuMeterValue()
{
    ...
    {
        if (vuMeterInL)
            vuMeterInL->setValueNormalized(mainController->getVuMeterByTag(VuMeter_inL));
        if (vuMeterInR)
            vuMeterInR->setValueNormalized(mainController->getVuMeterByTag(VuMeter_inR));
        if (vuMeterOutL)
            vuMeterOutL->setValueNormalized(mainController->getVuMeterByTag(VuMeter_outL));
        if (vuMeterOutR)
            vuMeterOutR->setValueNormalized(mainController->getVuMeterByTag(VuMeter_outR));
        if (vuMeterEffect)
            vuMeterEffect->setValueNormalized(mainController->getVuMeterByTag(VuMeter_effect));
    }
}

template <>
VSTGUI::CView*
JSIF_Controller::UIVuMeterController::verifyView(VSTGUI::CView* view,
...
)
{
    if (VSTGUI::CVuMeter* control = dynamic_cast<VSTGUI::CVuMeter*>(view); control)
    {
        ...
        {
            vuMeterEffect = control;
            vuMeterEffect->registerViewListener(this);
        }
    }
    return view;
}
...
template <> void JSIF_Controller::UIVuMeterController::viewWillDelete(VSTGUI::CView* view)
{
    ...
    if (dynamic_cast<VSTGUI::CVuMeter*>(view) == vuMeterEffect && vuMeterEffect)
    {
        vuMeterEffect->unregisterViewListener(this);
...
//------------------------------------------------------------------------
SliderParameter::SliderParameter(const Vst::TChar* title, int32 tag, const Vst::TChar* units,
                                 Vst::ParamValue minPlain, Vst::ParamValue maxPlain,
                                 Vst::ParamValue defaultValuePlain, int32 stepCount, int32 flags,
                                 Vst::UnitID unitID)
{
...
Vst::ParamValue SliderParameter::toPlain(Vst::ParamValue _valueNormalized) const
{
    ...
    if (_valueNormalized == 1.00)
        return 6.0;
    if (_valueNormalized >= 0.96)
        return 6.0;
    if (_valueNormalized >= 0.92)
        return 5.0;
    if (_valueNormalized >= 0.88)
        return 4.0;
    if (_valueNormalized >= 0.84)
        return 3.0;
    if (_valueNormalized >= 0.80)
        return 2.0;
    if (_valueNormalized >= 0.76)
        return 1.0;
    if (_valueNormalized >= 0.72)
        return 0.0;
    if (_valueNormalized >= 0.68)
        return -1.0;
    if (_valueNormalized >= 0.64)
        return -2.0;
    if (_valueNormalized >= 0.60)
        return -3.0;
    if (_valueNormalized >= 0.56)
        return -4.0;
    if (_valueNormalized >= 0.52)
        return -5.0;
    if (_valueNormalized >= 0.48)
        return -6.0;
    if (_valueNormalized >= 0.44)
        return -8.0;
    if (_valueNormalized >= 0.40)
        return -10.0;
    if (_valueNormalized >= 0.36)
        return -12.0;
    if (_valueNormalized >= 0.32)
        return -14.0;
    if (_valueNormalized >= 0.28)
        return -16.0;
    if (_valueNormalized >= 0.24)
        return -18.0;
    if (_valueNormalized >= 0.20)
        return -22.0;
    if (_valueNormalized >= 0.16)
        return -26.0;
    if (_valueNormalized >= 0.12)
        return -30.0;
    if (_valueNormalized >= 0.08)
        return -34.0;
    if (_valueNormalized >= 0.04)
        return -38.0;
    return -42.0;
}
//------------------------------------------------------------------------
...
tresult PLUGIN_API JSIF_Controller::initialize(FUnknown* context)
{
    ...
    defaultPlain = 0.0;
    stepCount = 0;
    auto* ParamIn = new Vst::RangeParameter(STR16("Input"), tag, STR16("dB"), minPlain, maxPlain,
                                            defaultPlain, stepCount, flags);
    ...
    defaultPlain = 0.0;
    stepCount = 0;
    auto* ParamOut = new Vst::RangeParameter(STR16("Output"), tag, STR16("dB"), minPlain, maxPlain,
                                             defaultPlain, stepCount, flags);
    ...
    defaultPlain = 0;
    stepCount = 0;
    auto* ParamEffect = new Vst::RangeParameter(STR16("Effect"), tag, STR16("%"), minPlain,
                                                maxPlain, defaultPlain, stepCount, flags);
    ...
    defaultPlain = 0;
    stepCount = 0;
    auto* ParamCurve = new Vst::RangeParameter(STR16("Curve"), tag, STR16("%"), minPlain, maxPlain,
                                               defaultPlain, stepCount, flags);
    ...
    for (ZoomFactorVector::const_iterator it = zoomFactors.begin(), end = zoomFactors.end();
         it != end; ++it)
    {
    ...
    Vst::StringListParameter* GuiSwitch =
        new Vst::StringListParameter(STR("GUI Switch"), kGuiSwitch);
...
tresult PLUGIN_API JSIF_Controller::setComponentState(IBStream* state)
{
    ...
    if (streamer.readDouble(savedInput) == false)
        return kResultFalse;
    if (streamer.readDouble(savedEffect) == false)
        return kResultFalse;
    if (streamer.readDouble(savedCurve) == false)
        return kResultFalse;
    if (streamer.readDouble(savedOutput) == false)
        return kResultFalse;
    if (streamer.readDouble(savedOS) == false)
        return kResultFalse;
    if (streamer.readInt32(savedClip) == false)
        return kResultFalse;
    if (streamer.readInt32(savedIn) == false)
        return kResultFalse;
    if (streamer.readInt32(savedSplit) == false)
        return kResultFalse;
    if (streamer.readDouble(savedZoom) == false)
        return kResultFalse;
    if (streamer.readDouble(savedPhase) == false)
        return kResultFalse;
...
{
    // Loading UI state independently <- reload because of AUv2 overwrites GUI parameters to default
    // AFTER setComponentState.

    // Moreover, in case of Bitwig, createView is called before setState.
    // So saved GUI type state is not correctly reproduced.
    // Also, Editors are added while GUI is opening -> so have to manually save created base view
    // and call exchangeView.
    ...
    if (streamer.readDouble(savedPhase) == false)
        savedPhase = 0.0;
    if (streamer.readDouble(savedGUI) == false)
        savedGUI = 0.0;
...
VSTGUI::IController* JSIF_Controller::createSubController(VSTGUI::UTF8StringPtr name,
                                                          const VSTGUI::IUIDescription* description,
...
void PLUGIN_API JSIF_Controller::update(FUnknown* changedUnknown, int32 message)
{
    ...
    if (param->getInfo().id == kParamZoom)
    {
        ...
        for (EditorVector::const_iterator it = editors.begin(), end = editors.end(); it != end;
             ++it)
        {
             ...
             ++it)
        {
            ...
            */
            ...
            if (editor)
            {
                ...
                {
                    if (stateGUI == 0.0)
                        editor->exchangeView("Original");
                    else if (stateGUI == 1.0)
                        editor->exchangeView("Twarch");
...
void JSIF_Controller::editorAttached(Steinberg::Vst::EditorView* editor)
{
    ...
    if (_editor)
    {
        ...
        {
            if (stateGUI == 0.0)
                _editor->exchangeView("Original");
            else if (stateGUI == 1.0)
                _editor->exchangeView("Twarch");
...
//------------------------------------------------------------------------
tresult PLUGIN_API JSIF_Controller::getParamStringByValue(Vst::ParamID tag,
...
//------------------------------------------------------------------------
tresult PLUGIN_API JSIF_Controller::getParamValueByString(Vst::ParamID tag, Vst::TChar* string,
                                                          Vst::ParamValue& valueNormalized)
{
...
tresult PLUGIN_API JSIF_Controller::notify(Vst::IMessage* message)
{
    ...
    if (strcmp(message->getMessageID(), "VUmeter") == 0)
    {
        double data = 0.0;
        int64 update = 0.0;
        if (message->getAttributes()->getFloat("vuInL", data) == kResultTrue)
            vuInL = data;
        if (message->getAttributes()->getFloat("vuInR", data) == kResultTrue)
            vuInR = data;
        if (message->getAttributes()->getFloat("vuOutL", data) == kResultTrue)
            vuOutL = data;
        if (message->getAttributes()->getFloat("vuOutR", data) == kResultTrue)
            vuOutR = data;
        if (message->getAttributes()->getFloat("vuEffect", data) == kResultTrue)
            vuEffect = data;
        if (message->getAttributes()->getInt("update", update) == kResultTrue)
        {
            if (!vuMeterControllers.empty())
            {
                for (auto iter = vuMeterControllers.begin(); iter != vuMeterControllers.end();
                     iter++)
                {
